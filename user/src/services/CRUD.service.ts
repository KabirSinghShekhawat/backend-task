...
const createOne = (Model: mongoose.Model<any>, filter: Array<string>) => async (
    ...
    let query = Model.findById(newDocument.id)
    if (req.query?.select) query = query.select(req.query.select)

    const result = await query;

    await axios.post(EventBus, { type: 'UserCreated', data: {_id: result._id}});
...
const deleteOne = (Model: mongoose.Model<any>) => async (
    ...
    await axios.post(EventBus, {type: 'UserDeleted', data: {_id: documentID}});
    res
        .status(StatusCodes.CREATED)
        .json(
            createResponse(
